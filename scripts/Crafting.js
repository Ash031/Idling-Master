...
function craft(itemN){
    ...
    for(var i = 0;i<item.cost.length;i++){
        if(ores[item.cost[i].ore].quant<Math.floor(Math.pow(item.level,1.5)*item.cost[i].baseCost)) return;
    }
    for(var i = 0;i<item.cost.length;i++){
        ores[item.cost[i].ore].quant-=Math.floor(Math.pow(item.level,1.5)*item.cost[i].baseCost);
    }
